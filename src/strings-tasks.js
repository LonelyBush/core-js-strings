...
function removeFirstOccurrences(str, value) {
  const cuting = str.indexOf(value);
  if (cuting === -1) {
    return str;
  }
  const cutOff = str.slice(0, cuting) + str.slice(cuting + value.length);
  return cutOff;
}
...
function removeLastOccurrences(str, value) {
  const cuting = str.lastIndexOf(value);
  if (cuting === -1) {
    return str;
  }
  const cutOff = str.slice(0, cuting) + str.slice(cuting + value.length);
...
function sumOfCodes(str) {
  ...
  const sumOfChars = newArr.reduce((accum, curr) => {
    return accum + curr.charCodeAt();
  }, 0);
  return sumOfChars;
}
...
function formatTime(minutes, seconds) {
  const toStrMin = `${minutes}`;
  const toStrSec = `${seconds}`;
  if (toStrMin < 10 || toStrSec < 10) {
    return `${toStrMin.padStart(2, '0')}:${toStrSec.padStart(2, '0')}`;
  }
  return `${toStrMin}:${toStrSec}`;
}
...
function isPalindrome(str) {
  const newArr = str
    ...
    .filter(
      (elem) =>
        elem.trim() !== '' && elem !== ',' && elem !== '?' && elem !== '!'
    );
  // eslint-disable-next-line no-self-compare
  if (newArr.reverse().join('') === newArr.reverse().join('')) {
    return true;
  }
  return false;
}
...
function findLongestWord(sentence) {
  ...
  const filtered = newArr.reduce((accur, curr) =>
    accur.length >= curr.length ? accur : curr
  );
  return filtered;
}
...
function reverseWords(str) {
  ...
  const fors = sent.map((elem) => elem.split('').reverse().join(''));
  return fors.join(' ');
}
